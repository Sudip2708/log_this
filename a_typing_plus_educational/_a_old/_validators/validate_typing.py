...
from ..validate_condition import ValidateCondition

TypingType = Any

class ValidateTyping:
    ...
    def _get_annotation_key(annotation) -> str:
        try:
            stringified = str(annotation)
            index = stringified.find('[')
            if index != -1:
                stringified = stringified[:index]
            return stringified.lower().strip()
        except Exception:
