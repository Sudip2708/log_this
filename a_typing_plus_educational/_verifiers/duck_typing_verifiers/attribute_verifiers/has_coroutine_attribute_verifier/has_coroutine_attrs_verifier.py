    ...
    try:
        ...
        for attribute_name in attribute_names:
            ...
            try:
                ...
                if has_coroutine_attr_verifier(obj, attribute_name, bool_only=True):
                    outcome["attributes"].append(attribute_name)

                # Pokud ověření je negativní
                outcome["errors"]["not_coroutine_function"].append(attribute_name)
        ...
        if (
            ...
            raise VerifyAttributesNotCoroutineFunctionError(obj, attribute_names, outcome)

        # Pokud objekt obsahuje všechny atributy
        return True

    # Přeposlání vnitřní výjimky
    except VerifyAttributesNotCoroutineFunctionError:
        raise
