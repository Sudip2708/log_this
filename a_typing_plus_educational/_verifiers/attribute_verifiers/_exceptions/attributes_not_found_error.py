...
class VerifyAttributesNotFoundError(VerifyValueError, AttributeError):

    # Specifický nadpis pro chyby nenalezených atributů
    title = "\n⚠ OBJEKT NEOBSAHUJE VŠECHNY POŽADOVANÉ ATRIBUTY!\n"
    ...
    def __init__(
        ...
        what_happened = [
            "   - Funkce pro získání atributu objektu zjistila, že objekt neobsahuje všechny požadované atributy.\n",
            f"   - Objekt: {repr(self.obj)}\n",
            f"   - Požadované atributy: {self._format_items(self.attribute_names)}\n",
        ...
        if self.absent:
            what_to_do.append(
                "   - Zkontroluj objekt a ověř, zda má obsahovat požadované atributy.\n",
            )
