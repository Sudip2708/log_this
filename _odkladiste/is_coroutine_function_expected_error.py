...
class VerifyIsCoroutineFunctionExpectedError(VerifyParameterError):
    """
    Výjimka vyvolaná při neplatné specifikaci očekávaných atributů pro coroutine funkce.

    Tato výjimka je vyvolána, když parametr `expected_attribute` předaný do funkce
    `is_coroutine_function_verifier()` nemá očekávaný formát. Očekává se, že tento
    parametr bude buď řetězec s názvem jednoho atributu, nebo iterovatelná kolekce
    řetězců obsahujících názvy atributů, které by měly být coroutine funkcemi.
    ...
    * expected (Any): Původní, chybně zadaná hodnota parametru `expected_attribute`
    ...
    class MyObject:
        def __init__(self):
            self.async_method = my_coroutine
    ...
    try:
        ...
        is_coroutine_function_verifier(obj, my_coroutine)  # Chyba: Předán coroutine objekt místo názvu atributu
    except VerifyIsCoroutineFunctionExpectedError as e:
        print(f"Chyba specifikace atributu: {e}")
        # Náprava problému
    ```

    Souvislosti s ověřováním atributů pro asynchronní funkce:
    --------------------------------------------------------
    Pro správné ověření, zda objekt obsahuje atributy, které jsou coroutine
    funkcemi, je nutné poskytnout názvy těchto atributů ve správném formátu.
    Tato výjimka pomáhá uživatelům knihovny vyhnout se běžným chybám při této
    specifikaci, jako je například předání samotného objektu atributu místo jeho názvu.
    """
    ...
    def __init__(self, expected_attribute: Any):
        ...
        pro funkčnost ověřování přítomnosti a typu atributů (zda jsou coroutine funkcemi).
        """
        ...
        what_happened = [
            "   - Byl předán neplatný vstup definující atributy pro ověření coroutine function.\n",
        ...
        what_to_do = [
            ...
            "   - Správně: is_coroutine_function_verifier(value, 'attribute_name')  # použití řetězce\n",
            "   - Špatně: is_coroutine_function_verifier(value, attribute_name)  # zapomenutí apostrofů\n",
