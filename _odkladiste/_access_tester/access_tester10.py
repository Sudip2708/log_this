...
class AccessTester:
    ...
    def _can_save_file(self):
        ...
        try:
            ...
            # Zaznamenání vytvořní souboru do atributu
            self._test_file_is_created = True
        except PermissionError:
    ...
    def _can_delete_file(self):
        try:
            # Reset atributu před smazáním (pro případ, kdyby smazání selhalo, tak ať neprobíhá dvakrát)
            self._test_file_is_created = False
            ...
            cli_print.warning.text(f"Popis chyby: {e} ")
            cli_print.warning.direction(f"Cesta k souboru: {self._test_file_path} ")
            return False

    # Konečný reset nastavení
    def _final_clean_up(self):
        if self._test_file_is_created:
            self._can_delete_file()
        self._path = None
