...
class AccessTester:
    ...
    def _is_dir_present(self):
        try:
            ...
            self._permission_error_handler()
        except (OSError, Exception) as self._e:
            cli_print.warning.text("Nepovedlo se vytvořit cílovou složku. ")
            if OSError:
                self._os_error_handler()
            if Exception:
    ...
    def _can_read_file(self):
        try:
            ...
            if FileNotFoundError:
                self._file_not_found_error_handler()
            if PermissionError:
                self._permission_error_handler()
        except (OSError, Exception) as self._e:
            cli_print.warning.text(
    ...
    def _can_delete_file(self):
        try:
            ...
            cli_print.warning.text(
                "Vyskytla se chyba při smazání testovacího souboru. ")
            if OSError:
                self._os_error_handler()
            if Exception:
                self._exception_handler()
    ...
    def _file_not_found_error_handler(self):
