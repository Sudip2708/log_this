...
from methods.menus import (
    ...
    MenusConfigMixin, MenusSymbolsSettingsMixin, MenusColorsSettingsMixin
)

from .menus_class.get_menus_dict import get_menus_dict
...
class InteractiveCli(
    ...
    # Používá metody: show_menu(), run_menu()
    ...
    # Používá atributy: 'response', 'selected_key', 'selected_value'
    # Používá metody: show_menu(), run_menu(), input_custom_int_value()
    ...
    def __init__(self, menu_name):
        ...
        if not hasattr(self, "_initialized"):
            ...
            # Slovník s odkazy na třídy s menu
            self.menus_dict = get_menus_dict(self)
    ...
    def get_menu_data(self):
        """Na základě 'menu_name' načte data pro požadované menu"""
        if self.menu_name in self.menus_dict:
            return self.menus_dict[self.menu_name]()


    def get_menu_attributes(self):
        """Na základě 'menu_name' vrátí obsah pro 'menu_title' a 'menu_items'"""
        title, items, selected = self.get_menu_data()
