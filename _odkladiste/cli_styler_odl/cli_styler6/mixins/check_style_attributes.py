...
class CheckStyleAttributesMixin(ABC):
    ...
    def check_style_attributes(self, styles_dict):
        ...
        if attributes_set != styles_set:
            self._raise_attributes_error(attributes_set, styles_set)

    def _get_style_attributes_set(self):
        """
        Metoda pro vytvoření množiny atributů pro styly definované v této třídě.

        - Načte atributy třídy (vynechá metody).
        - Odstraní atributy začínající podtržítkem.
        - Ujistí se, že `init` není součástí.
        """
        return {
            key for key in dir(self)
            if not key.startswith("_")  # Odstranění speciálních atributů
               and key != "init"  # Ujistíme se, že `init` není součástí
               and not callable(getattr(self, key))  # Odstranění metod
        }

    @staticmethod
    def _raise_attributes_error(attributes_set, styles_set):
