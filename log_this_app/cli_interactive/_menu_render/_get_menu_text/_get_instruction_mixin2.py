...
class GetInstructionMixin(ABC):
    ...
    def _get_instruction(self):
        ...
        if self.mm.show_instruction:
            instruction.extend(self._get_base_instruction())
            instruction.extend(self._get_menu_instruction())
        return instruction
    ...
    def _get_menu_instruction(self) -> List[Tuple[str, str]]:
        menu = self.mm.menu_register(self.mm.menu_name)
        menu_help = menu.menu_help

        title = styler.get_style.hint.title(
        ...
        return [title, *texts] if menu_help else []
