...
class ValidateTypingData:
    ...
    def _verify_list(self, inner_types):
        self.validate_native_type(self.value, list)
        if not self.inner_check:
            return True
        for item in self.value:
            new_check = self.get_new_inner_check()
            self(item, inner_types[0], new_check)
        return True
    ...
    def validate_typing_annotation(self):
        ...
        if not self.inner_check and isinstance(origin, type):
            return self.validate_native_type(self.value, origin)

        # Zpracování všech ostatních případů
        # Např. List[str]
        if origin in self.TYPING_COMMANDS:
            method_name = self.TYPING_COMMANDS[origin]
            return getattr(self, method_name)(args)
        else:
            raise TypeError(f"Nepodporovaný generický typ: {origin}")
