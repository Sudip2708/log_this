...
class GetSimplifiedTracebackInputError(TypeError):
    ...
    def __init__(self, parameter, input_value, allowed_value, description):
        ...
        super().__init__(self.message) # Předání zprávy rodičovské třídě

    def __str__(self):
        return self.message # Pro lepší výpis při tisku výjimky
...
class GetSimplifiedTraceback:
    ...
    def _get_skip_lines(skip_lines):
        ...
        if not isinstance(skip_lines, int) and skip_lines < 0:
            raise GetSimplifiedTracebackInputError(
                ...
                description="Určuje počet záznamů od konce, které se mají ve výpisu vynechat."
            )
        return skip_lines
    ...
    def _get_users_code_only(users_code_only):
        if not isinstance(users_code_only, bool):
            raise GetSimplifiedTracebackInputError(
                ...
                description="Určuje zda se mají vypsat pouze uživatelovo záznamy, nebo všechny."
            )
    ...
    def _get_lines_limit(lines_limit):
        if not isinstance(lines_limit, bool) or (
            ...
            raise GetSimplifiedTracebackInputError(
                ...
                allowed_value="False pro všechny záznamy, jinak celé kladné číslo pro definici počtu.",
    ...
    def __call__(
        ...
        self._skip_lines = self._get_skip_lines(skip_lines)
        self._users_code_only = self._get_users_code_only(users_code_only)
        self._lines_limit = self._get_lines_limit(lines_limit)
    ...
    def _trace_lines(self):
        ...
        for frame in self._stack_summary:
            ...
            if self._lines_limit and self._lines_count > self._lines_limit:
                break

            # Podmínka kontrolující zda se mají vypysovat jen uživatelské záznami
            if self._users_code_only and not self._is_user_code(frame.filename):
                pass
            else:
                trace_lines.append(
                    f"   - {frame.line}"
