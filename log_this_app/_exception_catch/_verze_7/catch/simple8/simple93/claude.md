Rozumím tvé myšlence. Chtěl bys analyzovat typové anotace a rozložit je na jednoduché komponenty, které by pak šlo snadno porovnávat s hodnotami. Vytvořím pro tebe kód, který rozloží (parsuje) typové anotace z knihovny typing.
...
def parse_typing_annotation(annotation):
    ...
    if origin is typing.Union and type(None) in args:
        ...
        if len(non_none_args) == 1:
            ...
            return {"optional": {"union": tuple(parse_typing_annotation(arg) for arg in non_none_args)}}
    ...
    if origin in (tuple, typing.Tuple):
        if len(args) == 0:
            ...
            return {"tuple": tuple(parse_typing_annotation(arg) for arg in args)}
    
    if origin in (dict, typing.Dict):
        if len(args) == 0:
            return {"dict": ("Any", "Any")}
        elif len(args) == 2:
            return {"dict": (parse_typing_annotation(args[0]), parse_typing_annotation(args[1]))}
    ...
    if origin is typing.Callable:
        if len(args) == 0 or args[0] is Ellipsis:
            return {"callable": ("Any", parse_typing_annotation(args[1]) if len(args) > 1 else "Any")}
        else:
    ...
    if inspect.isclass(annotation) and hasattr(annotation, "__annotations__"):
        if issubclass(annotation, typing.TypedDict) or issubclass(annotation, tuple) and hasattr(annotation, "_field_types"):
            field_types = {}
    ...
    if origin is not None:
        ...
        if len(args) == 0:
            ...
            return {origin_name.lower(): tuple(parse_typing_annotation(arg) for arg in args)}
    ...
    if isinstance(parsed_type, dict):
        ...
        if "list" in parsed_type:
            ...
            if isinstance(parsed_type["list"], tuple):
                ...
                if len(value) != len(parsed_type["list"]):
                    return False
                return all(validate_value(value[i], parsed_type["list"][i]) for i in range(len(value)))
            else:
                # Homogenní list s jedním typem pro všechny prvky
                return all(validate_value(item, parsed_type["list"]) for item in value)
        ...
        if "tuple" in parsed_type:
            ...
            if isinstance(parsed_type["tuple"], dict) and "repeat" in parsed_type["tuple"]:
                ...
                return all(validate_value(item, parsed_type["tuple"]["repeat"]) for item in value)
            if isinstance(parsed_type["tuple"], tuple):
                # Heterogenní tuple (Tuple[T1, T2, ...])
                if len(value) != len(parsed_type["tuple"]):
                    return False
                return all(validate_value(value[i], parsed_type["tuple"][i]) for i in range(len(value)))
        ...
        if "dict" in parsed_type:
            ...
            return all(validate_value(k, key_type) and validate_value(v, value_type) for k, v in value.items())
        ...
        if "set" in parsed_type:
            ...
            if parsed_type["set"] == "Any":
                return True
            return all(validate_value(item, parsed_type["set"]) for item in value)
...
Vytvořil jsem pro tebe komplexní knihovnu pro parsování a ověřování typových anotací z modulu `typing`. Kód obsahuje dvě hlavní funkce:

1. `parse_typing_annotation()` - tato funkce rozloží komplexní typové anotace na zjednodušenou strukturu, se kterou lze snáze pracovat
2. `check_type()` a `validate_value()` - tyto funkce ověřují, zda zadaná hodnota odpovídá dané typové anotaci

### Příklady parsování typových anotací:

```python
parse_typing_annotation(typing.List[str])
# Výsledek: {'list': 'str'}

parse_typing_annotation(typing.Union[int, str])
# Výsledek: {'union': ('int', 'str')}

parse_typing_annotation(typing.Tuple[int, str, bool])
# Výsledek: {'tuple': ('int', 'str', 'bool')}
```

### Příklady kontroly typů:

```python
check_type(["hello", "world"], typing.List[str])  # True
check_type(["hello", 123], typing.List[str])  # False
check_type([1, 2, 3], typing.List[typing.Union[int, float]])  # True
```

Funkce podporuje většinu běžně používaných typů z modulu `typing`, včetně:
- List, Dict, Tuple, Set
- Union, Optional
- Literal
- Callable
...
Kód je navržen modulárně, takže můžeš snadno přidat podporu pro další typy nebo upravit stávající chování podle potřeby.

Pokud bys chtěl tuto knihovnu rozšířit, mohl bys přidat:
1. Lepší podporu pro generické typy s parametry
2. Podrobnější kontrolu pro Callable a funkční signatury
3. Pokročilejší kontrolu pro Protocol a strukturální typování
4. Chybové zprávy, které vysvětlují, proč hodnota neodpovídá typu

Máš nějaké otázky k tomuto kódu nebo potřebuješ nějaké úpravy?