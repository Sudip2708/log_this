...
class InteractiveMode:
    ...
    def _handle_config_change(self) -> None:
        ...
        key = self._select_config_key()
        if not key:
            return

        # Pak zobrazíme dialog pro zadání hodnoty
        value = self._input_config_value(key)
        if value is None:
            return

        # Provedeme validaci a uložení
        if self._validate_and_save(key, value):
            self._show_success_menu(key, value)

    def _select_config_key(self) -> Optional[str]:
        """
    ...
    def _input_config_value(self, key: str) -> Optional[Any]:
        """
