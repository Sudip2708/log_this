...
class MargeWithDefaultsMixin(ABC):
    """Mixin metody pro doplnění chybějících klíčů"""

    @property
    @abstractmethod
    def defaults(self) -> Dict[str, Union[int, str, bool]]:
        """Property vracející slovník s výchozími hodnotami."""
        pass
    ...
    def _merge_with_defaults(
        ...
        try:
            ...
            updated_config = self.defaults.copy()

            # Přepsání defaultních hodnot hodnotami ze slovníku
            updated_config.update({
                key: config_dict.get(key, default)
